package Methpup::Runnable::GeneDropoutPlot;
...
sub run{
	...
	my $fcmd = "find $idir -name '$fpattern' | xargs cat > ${rdir}gene2read.tab";
	$self->verbose($fcmd);
	`$fcmd`;
	## in.file out.dir
	my @params = $self->inputs->{script};
	push @params,"in.file=\\'${rdir}gene2read.tab\\'";
...
sub run_conditions{
	my $self=shift;
	return 1 if -e  $self->inputs->{out_dir}."/gene_dropout.pdf";
	return 0;
}
1;
